...
void drawAVerticalBezier(int z, int n)
{
  fill(int(random(255)), int(random(255)), int(random(255)), 77);
...
void drawAHorizontalBezier(int z, int n)
{
  fill(int(random(255)), int(random(255)), int(random(255)), 77);
  // Sets fill to no so that Beziers are pure Lines
  // And Beziers not hide ech oher
...
void generate()
{
  ...
  for(int z = 0; z < zx; z++) drawAVerticalBezier(z, zx);
  for(int z = 0; z < zy; z++) drawAHorizontalBezier(z, zy);
}
